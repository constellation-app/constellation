...
@ServiceProvider(service = ContextMenuProvider.class, position = 1200)
public class LayersRemoveContextMenu implements ContextMenuProvider {
    ...
    public void selectItem(final String item, final Graph graph, final GraphElementType elementType, final int elementId, final Vector3f unprojected) {
        PluginExecution.withPlugin(new UpdateElementBitmaskPlugin(Integer.parseInt(item), LayerAction.REMOVE))
