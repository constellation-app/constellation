 ...
 * @author antares
 */
public class ContractCompositePluginNGTest {
    ...
    /**
     * Test of getItems method, of class ContractCompositePlugin.
     */
    @Test
    public void testGetItems() {
        System.out.println("getItems");
        
        final ContractCompositePlugin instance = new ContractCompositePlugin();
        
        // vxId1 is already a contracted composite so no options should be presented
        final List<String> result1 = instance.getItems(graph, GraphElementType.VERTEX, vxId1);
        assertTrue(result1.isEmpty());
        
        // vxId2 is a plain old node so no options should be presented
        final List<String> result2 = instance.getItems(graph, GraphElementType.VERTEX, vxId2);
        assertTrue(result2.isEmpty());
        
        // vxId3 is an expanded composite so this should give the option to contract
        final List<String> result3 = instance.getItems(graph, GraphElementType.VERTEX, vxId3);
        assertEquals(result3.size(), 1);
        assertEquals(result3.get(0), "Contract Composite");
    }
}
