 ...
 * @author sirius
 */
@Messages("BoxSelectorPlugin=Select in Box")
@PluginInfo(pluginType = PluginType.SELECTION, tags = {"SELECTION"})
public final class BoxSelectorPlugin extends SimpleEditPlugin {

    private final boolean isAdd, isToggle;
    private final Camera camera;
    private final float[] box; // left, right, top, bottom in camera coordinates

    public BoxSelectorPlugin(final boolean isAdd, final boolean isToggle, final Camera camera, final float[] box) {
        this.isAdd = isAdd;
    ...
    public void edit(final GraphWriteMethods graph, final PluginInteraction interaction, final PluginParameters parameters) throws InterruptedException {
        ...
        for (int position = 0; position != vxCount; position++) {
            ...
            float x = xAttr != Graph.NOT_FOUND ? graph.getFloatValue(xAttr, vxId) : VisualDefaults.getDefaultX(vxId);
            float y = yAttr != Graph.NOT_FOUND ? graph.getFloatValue(yAttr, vxId) : VisualDefaults.getDefaultY(vxId);
            float z = zAttr != Graph.NOT_FOUND ? graph.getFloatValue(zAttr, vxId) : VisualDefaults.getDefaultZ(vxId);
        ...
        if (vxIncluded.isEmpty()) {
            ...
            for (int position = 0; position < linkCount; position++) {
                ...
                float xLo = xAttr != Graph.NOT_FOUND ? graph.getFloatValue(xAttr, vxLo) : VisualDefaults.getDefaultX(vxLo);
                float yLo = yAttr != Graph.NOT_FOUND ? graph.getFloatValue(yAttr, vxLo) : VisualDefaults.getDefaultY(vxLo);
                float zLo = zAttr != Graph.NOT_FOUND ? graph.getFloatValue(zAttr, vxLo) : VisualDefaults.getDefaultZ(vxLo);

                // Get the main location of the lo vertex.
                float xHi = xAttr != Graph.NOT_FOUND ? graph.getFloatValue(xAttr, vxHi) : VisualDefaults.getDefaultX(vxHi);
                float yHi = yAttr != Graph.NOT_FOUND ? graph.getFloatValue(yAttr, vxHi) : VisualDefaults.getDefaultY(vxHi);
                float zHi = zAttr != Graph.NOT_FOUND ? graph.getFloatValue(zAttr, vxHi) : VisualDefaults.getDefaultZ(vxHi);
