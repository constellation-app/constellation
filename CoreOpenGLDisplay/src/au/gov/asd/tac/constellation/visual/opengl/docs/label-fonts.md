...
ends with a type (that is, ends with \"\<\...\>\"), the text of the type
...
file containing the font. OpenType font files ([.otf]{.mono}) and some
TrueType font files ([.ttf]{.mono}) must be specified using the
filename.

On Windows, if an OTF font file name such as
[NotoSerifSC-Regular.otf]{.mono} is specified, the file will first be
looked for in the user\'s local profile font directory
[%LOCALAPPDATA%/Microsoft/Windows/Fonts/NotoSerifSC-Regular.otf]{.mono},
then the windows directory
[%WINDIR%/Fonts/NotoSerifSC-Regular.otf]{.mono}.

An absolute filename ([C:/MyFonts/font1.ttf]{.mono} or
[/fonts/font1.ttf]{.mono}) can be used to specify a font file.

Blank lines and lines beginning with [\#]{.mono} are ignored.
...
New]{.mono}, then the installed font [Noto Sans Arabic]{.mono}, and
finally the font in the file
[C:\\Users\\User1\\Downloads\\Fonts\\NotoSerifCJKtc-Regular.otf]{.mono}.

### Font tags

Fonts can be tagged with additional information. Adding one of the tags
[BOLD]{.mono}, [ITALIC]{.mono}, or [BOLD_ITALIC]{.mono} will have the
...
This is an alternative to the previous configuration: the \"!\"
signifies a negation, so Arial Bold will not be used for Arabic
codepoints, which will therefore be drawn using Noto Sans Arabic.

### Fallback font

Regardless of the fonts that are specified, CONSTELLATION appends the
Java font [SansSerif]{.mono} to the font list. (On Windows this is
typically Arial combined with some other fonts.) Even if the list is
empty, or the fonts in the list cannot display all of the codepoints
required for labels, CONSTELLATION will always attempt to use
[SansSerif]{.mono}. This does not guarantee that all codepoints can be
displayed
