...
public class GlyphRectangleBufferNGTest {
    ...
    /**
     * Test that correct behavior occurs when we add a new image that/       
     * does will not exceed the width of the current line or the height
     * of the buffer
     */
    @Test
    ...
    /**
     * Test that when we add the same image twice it doesnt add a new image
     * to the buffer and return the index of the original
     */
    @Test
    public void testAddRectSameImage() {               
     ...
     /**
      * Now we will check that a new line is created when required
      * Triggers if (x + w + PADDING) >= width that is if there is not enough
      * space on the x axis to fit the new image and buffer.
      *
      * It should add like  carriage return on a typewriter, resetting the
      * x,y coordinates to the left of the buffer and below the current line
      * Add new image will will exceed the width as described above.
      */
    @Test
     ...
     * Triggers if (y + h + PADDING) >= height that is if there is not
     * enough space on the y axis to fit the new image and buffer.
     *
