...
public final class ZonedDateTimeAttributeDescription extends AbstractObjectAttributeDescription<ZonedDateTime> {
     ...
     * <p>
     * Strings using Z as a zone (yyyy-MM-ddTHH:mm:ss.SSSZ) are also accepted. (No checking is done: Other letters than
     * Z are also accepted, but are read as UTC.)
     * <p>
     * Parsing isn't strict: the date 2011-99-01 will be accepted and punctuation isn't checked. Since the context is
     ...
     * <p>
     * Note that this method directly reads substrings with static indices, as this is significantly faster than using a
     * ZonedDateTimeFormatter.
     *
     * @param string A String in CONSTELLATION's date-time format (yyyy-MM-ddTHH:mm:ss.SSS[zone]) to be parsed.
     *
    ...
    public ZonedDateTime convertFromString(final String string) {
        if (StringUtils.isBlank(string)) {
            ...
            try {
                ...
                final String millisecondString = (string.length() > TemporalFormatting.DATE_MILLISECOND_FORMAT_START_POSITION && string.length() >= TemporalFormatting.DATE_TIME_FORMAT_LENGTH)
                        ? string.substring(TemporalFormatting.DATE_MILLISECOND_FORMAT_START_POSITION, TemporalFormatting.DATE_TIME_FORMAT_LENGTH)
                        : "0";
                final int millisecond = Integer.parseInt(millisecondString, 10);
