...
first \'version\' section) consists of lists of dictionaries with a
single key. The top level array contains \'graph\', \'vertex\',
\'transaction\', and \'meta\', each containing a two-element list: a
dictionary with the key \'attrs\', and a dictionary with the key
\'data\'.

Suppose we want to find the names of nodes that have a secondary country
of \'Canada\'. The following function will find those nodes. (The
function is shown in two variants: one using a list comprehension, and
one using long form. Use whichever variant you are most comfortable
with.
...
Note the use of [vx.get()]{.tt} instead of using standard dictionary
lookup notation [vx\[\]]{.tt}. This is because a vertex attribute with a
null value is not written to the JSON document, and therefore does not
have a key in the Python dictionary when it is read. Attempting to read
a dictionary with a non-existent key results in a KeyError being thrown,
...
queries, such as \"all countries starting with \'U\'\", or \"events
between two dates\".
