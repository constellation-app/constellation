 ...
 * {@link au.gov.asd.tac.constellation.graph.attribute.ByteObjectAttributeDescription}
 *
 * @author cygnus_x-1
 */
@ServiceProvider(service = AbstractGraphIOProvider.class)
public class ByteObjectIOProvider extends AbstractGraphIOProvider {
    ...
    public void writeObject(final Attribute attribute, final int elementId, final JsonGenerator jsonGenerator,
        ...
        if (verbose || !graph.isDefaultValue(attribute.getId(), elementId)) {
            final Byte attributeValue = graph.getObjectValue(attribute.getId(), elementId);
            if (attributeValue != null) {
                jsonGenerator.writeNumberField(attribute.getName(), attributeValue.intValue());
            } else {
