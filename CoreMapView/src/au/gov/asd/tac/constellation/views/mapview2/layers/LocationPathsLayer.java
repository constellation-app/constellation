                    ...
                    for (int neighbourPosition = 0; neighbourPosition < neighbourCount; neighbourPosition++) {
                        ...
                        if (neighbourType != null && neighbourType.isSubTypeOf(AnalyticConcept.VertexType.LOCATION)) {
                            ...
                            for (int j = 0; j < linkOutgoingTransactionCount; ++j) {
                                ...
                                final LineMarker l = new LineMarker(parent, parent.getNewMarkerID(), neighbourLinkId, (float) sourceLat, (float) sourceLon, (float) destLat, (float) destLon);
                                if (!parent.getAllMarkers().keySet().contains(coordinateKey)) {
                                    l.getMarker().setStrokeWidth(30 * MapDetails.MARKER_LINE_WIDTH * parent.getScaledMapLineWidth());
                                    l.getMarker().setStroke(new LinearGradient(0, 0, 1, 1, true, CycleMethod.NO_CYCLE, new Stop[] {new Stop(0, Color.LIMEGREEN), new Stop(1, Color.BLACK)}));
