...
import au.gov.asd.tac.constellation.graph.visual.framework.VisualAccess.ConnectionDirection;
...
public final class GraphVisualAccess implements VisualAccess {
    ...
    private void recalculateConnectionMode() {
        connectionMode = graphConnectionMode != Graph.NOT_FOUND ? accessGraph.getObjectValue(graphConnectionMode, 0) : VisualDefaults.DEFAULT_CONNECTION_MODE;
    }
    ...
    private void recalculateDecorators() {
        final VertexDecorators decorators = graphDecorators != Graph.NOT_FOUND ? accessGraph.getObjectValue(graphDecorators, 0) : VisualDefaults.DEFAULT_DECORATORS;
    ...
    public TransactionLineStyle getConnectionLineStyle(int connection) {
        TransactionLineStyle style = null;
        TransactionLineStyle mixStyle = TransactionLineStyle.SOLID;
        if (transactionLineStyle != Graph.NOT_FOUND) {
            switch (connectionElementTypes[connection]) {
                case LINK:
                    ...
                    for (int i = 0; i < accessGraph.getLinkTransactionCount(linkId); i++) {
                        final int transactionId = accessGraph.getLinkTransaction(linkId, i);
                        final TransactionLineStyle transStyle = accessGraph.getObjectValue(transactionLineStyle, transactionId);
