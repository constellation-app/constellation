...
servers. For example, the suffix \"[.com]{.mono}\" matches hosts with
the names \"[maps.google.com]{.mono}\", \"[facebook.com]{.mono}\", and
so on.
...
The checkbox \"Use defaults\", when selected, indicates that the
...
The names \"localhost\" and \"127.0.0.1\" are implicitly prepended to
the local hosts list.
...
Any host ending with \"[.au]{.mono}\", and the specific host
\"[sydney.com]{.mono}\", will use the proxy \"[au-proxy]{.mono}\", port
8080. Any host ending with \"[.com]{.mono}\" will use the proxy
\"[other-proxy]{.mono}\", port 8888.
...
and the specific list are both empty, and the default proxy is
\"[myproxy:8080]{.mono}\", then all hosts will be accessed via the proxy
\"[myproxy]{.mono}\" using port 8080.
