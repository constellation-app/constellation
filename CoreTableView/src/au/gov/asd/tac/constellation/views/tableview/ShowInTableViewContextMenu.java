...
public class ShowInTableViewContextMenu implements ContextMenuProvider {
    ...
    public void selectItem(String item, Graph graph, GraphElementType elementType, int elementId, Vector3f unprojected) {
        SwingUtilities.invokeLater(() -> {
            final TopComponent tc = WindowManager.getDefault().findTopComponent(TableViewTopComponent.class.getSimpleName());
            if (tc != null) {
                if (!tc.isOpened()) {
                    tc.open();
                }
