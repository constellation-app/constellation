...
public class TableToolbarNGTest {
    ...
    /**
     * Verifies that when the column visibility button is clicked, the {@link ColumnVisibilityContextMenu}
     * is created, initialized and shown.
     */
    ...
    /**
     * When the selected only mode button is pressed, the table switches between "Selected Only Mode"
     * ON and OFF. This verifies that as the button is pressed that
     * transition between ON and OFF happens and the update state plugin is executed triggering
     * the required changes.
     *
     * @param selectedOnlyModeInitialState the initial status of the "Selected Only Mode", the expected
     *     status after the button is pressed will be the inverse
    ...
    /**
     * When the element type button is pressed the tables state is switched between
     * VERTEX and TRANSACTION. This verifies that as the button is pressed that
     * transition happens and the update state plugin is executed triggering
     * the required changes. The buttons icon should also change to the element
     * type now set in the state.
     *
