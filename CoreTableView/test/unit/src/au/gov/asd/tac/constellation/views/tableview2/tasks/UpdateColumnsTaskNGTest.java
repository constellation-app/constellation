...
public class UpdateColumnsTaskNGTest {
    ...
    public void run() {
        ...
        updateColumnsTask.reset(columnReferenceMap, tableViewState, tableSelectionListener, selectedOnlySelectionListener);
    ...
    /**
     * Verifies that the listener maintains state and if the last change is equal to the current
     * one then the change is not iterated over.
     *
    ...
    /**
     * Sets up mocks so that the change will be iterated 3 times. For only one iteration
     * (the final one) will the code be able to pass the conditional that allows an
     * update to occur.
     * 
     * @param listener the listener to be tested
     */
