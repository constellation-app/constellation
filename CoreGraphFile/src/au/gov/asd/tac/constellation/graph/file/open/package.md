...
package is in the \"User Utilities\" module, which does not expose an
API, so we would need a dependency on the implementation. Bad. (2) There
are other things in the \"User Utilities\" module that we don\'t want.
(3) We need to change the source of FileChooser to make our filter have
