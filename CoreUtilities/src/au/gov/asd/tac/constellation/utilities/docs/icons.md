...
built-in icon with your own custom icon. Custom icons can be
[PNG]{.mono} or [JPG]{.mono} format.

In your [.CONSTELLATION]{.mono} directory is a a subdirectory
[Icons]{.mono}. When CONSTELLATION starts, it looks in this directory
for custom icons.

Icon names must be categorised. For example,
[Person.DoctorWho.png]{.mono} is in the [Person]{.mono} category,
whereas [Vehicle.DoctorWho.Tardis.png]{.mono} is in category
[Vehicle]{.mono}, subcategory [DoctorWho]{.mono}. The icon selection
dialog uses the categories to organise the icons, so an icon called
[MyIcon.png]{.mono} will not be used.
...
Icons are specified by name in the [icon]{.mono} attribute. For
instance, to display the custom Doctor Who icon above, use
[Person.DoctorWho]{.mono} as the [icon]{.mono} attribute value.

Icons can also be referred to by their alias, which is just the last
part of the name. This means that the [icon]{.mono} attribute value
[DoctorWho]{.mono} will also display the [Person.DoctorWho]{.mono} icon.

If there is more than one icon with the same alias (e.g.
[Person.DoctorWho]{.mono} and [TVSeries.DoctorWho]{.mono}, an arbitrary
decision will be made as to which one to use.

## Characters

Included in the built-in icons are the ASCII characters 32 (space) to
126 (tilde) in the [Char]{.mono} category. These icons are white on a
transparent background, so when they are used as values of the
[background_icon]{.mono} attribute, their color can be changed using the
[color]{.mono} attribute.
