...
\"Password or token\" input field and select \"Log in\". Alternatively,
you can use the command [jupyter notebook list]{.mono} at a command
prompt to see what notebook servers are running, and use an existing
token.
...
[jupyter-notebook]{.mono} at a command prompt in the correct directory
has exactly the same result. This is just a convenience provided by
CONSTELLATION.
