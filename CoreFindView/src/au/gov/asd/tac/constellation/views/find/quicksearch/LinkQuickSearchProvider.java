...
public class LinkQuickSearchProvider implements SearchProvider {
    ...
    private static final Logger LOGGER = Logger.getLogger(LinkQuickSearchProvider.class.getName());
    ...
    public void evaluate(final SearchRequest request, final SearchResponse response) {
        ...
        if (graph != null) {
            ...
            if (searchRequest.startsWith(QuickSearchUtilities.LEFT_BRACKET)) {
                ...
                if (codePos > QuickSearchUtilities.LH_SUB_BRACKET.length() && searchRequest.startsWith(QuickSearchUtilities.CIRCLED_L)) {
                    ...
                    // This is a recent search for a different category, so we can end the Link search here
                    return;
                }
            }            
            final String convertedSearch = !prevSearch.isEmpty() ? prevSearch : searchRequest;
            final QuickFindPlugin plugin = new QuickFindPlugin(GraphElementType.LINK, convertedSearch);
            ...
            for (final FindResult item : results) {
                if (item != null) {
                    ...
                    final String displayText = QuickSearchUtilities.CIRCLED_L + "  " + QuickSearchUtilities.replaceBrackets(item.toString()) + "   " + QuickSearchUtilities.LH_SUB_BRACKET + subscriptId + QuickSearchUtilities.RH_SUB_BRACKET;
            ...
            if (!matchList.isEmpty()) {
                ...
                final String displayText = QuickSearchUtilities.CIRCLED_L + "  " + QuickSearchUtilities.replaceBrackets(result.toString()) + "   " + QuickSearchUtilities.LH_SUB_BRACKET + subscriptId + QuickSearchUtilities.RH_SUB_BRACKET;
